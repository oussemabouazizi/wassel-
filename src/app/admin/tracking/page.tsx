...
export default function AdminTrackingPage() {
  ...
  useEffect(() => {
    ...
    const offlineIcon = L.divIcon({
      className: 'custom-marker',
      html: `<div style="width:24px;height:24px;border-radius:50%;background:#9CA3AF;border:2px solid white;box-shadow:0 1px 4px rgba(0,0,0,0.2);opacity:0.6;"></div>`,
      iconSize: [24, 24],
      iconAnchor: [12, 12],
    });
    ...
    deliveries.forEach((d) => {
      const icon = d.online_status === 'online' ? onlineIcon
        : d.online_status === 'busy' ? busyIcon
        : offlineIcon;
        ...
        .bindPopup(`
          <div style="min-width:160px">
            ...
            <p style="font-size:12px;margin:0">
              <span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:${d.online_status === 'online' ? '#22C55E' : d.online_status === 'busy' ? '#F59E0B' : '#9CA3AF'};margin-right:4px"></span>
              ${d.online_status}
            </p>
          </div>
        `);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <Card className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm text-[var(--color-text-secondary)]">Total Visible</p>
      ...
      {loading ? (
        ...
        <Card>
          <EmptyState
            ...
            description="Delivery persons will appear here when they go online and share their location"
          />
        </Card>
      ) : (
